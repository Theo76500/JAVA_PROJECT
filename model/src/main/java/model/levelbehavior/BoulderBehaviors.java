...
import model.levelmechanics.GameStateMechanics;

public class BoulderBehaviors {

	private ModelBehaviors modelBehaviors;
	private GameStateMechanics gameStateMechanics;
	
	public BoulderBehaviors(ModelBehaviors modelBehaviors, GameStateMechanics gameStateMechanics) {
		this.modelBehaviors = modelBehaviors;
		this.gameStateMechanics = gameStateMechanics;
	}

	public GameStateMechanics getGameStateMechanics() {return gameStateMechanics;}
	public void setGameStateMechanics(GameStateMechanics gameStateMechanics) {this.gameStateMechanics = gameStateMechanics;}
		    ...
		    if(levelTab[j][i] != null && levelTab[j][i].equals("Boulder") && (coordX == j) && (coordY == i+1)) {}
		    	...
		    	levelTab[j][i+1] = "Boulder";
		    		if(levelTab[j][i] != null && levelTab[j][i+1].equals("Boulder") && (coordX == j) && (coordY == i+2)) {
		    			this.getGameStateMechanics().GameOver();
				    }				    		
		    }    	
		}
	}
